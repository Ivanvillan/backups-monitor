...
<script>
    ...
    $('.dashboard').click(function (e) { 
        e.preventDefault();
        $('.row-dashboard').removeClass('hide');
        $('.row-triggers').addClass('hide');
        $('.row-clients').addClass('hide');
        $('.row-init').addClass('hide');
    });
        ...
        $.ajax({
            ...
            dataType: "json",
                success: function (response) {
                    ...
                    <td>${row[i].phone}</td> 
                    <td>${row[i].email}</td> 
                    <td><a href="#equipmentClient-modal" class="btn equipmentsClient cyan darken-2 modal-trigger"><i class="material-icons" style="color: #000 !important;">assignment</i></a></td>  
                    <td><a href="#" class="btn registerClient blue lighten-2"><i class="material-icons" style="color: #000 !important;">library_books</i></a></td>  
                    <td> <a href="#editClient" class="btn modal-trigger yellow darken-2 modalEditClient"><i class="material-icons" style="color: #000 !important;">edit</i></a></td> 
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                let row = response.result;
                    ...
                    <td><a href="#" class="btn cyan darken-2 taskList"><i class="material-icons" style="color: #000 !important;">assignment</i></a></td>  
                    <td><a href="#" class="btn showEditEquipment yellow darken-2"><i class="material-icons" style="color: #000 !important;">edit</i></a></td>  
        ...
        $.ajax({
            ...
            success: function (response) {
                let row = response.result;
                    ...
                    <td><a href="#" class="btn orange lighten-2 show-editTask"><i class="material-icons" style="color: #000 !important;">edit</i></a></td>  
                    <td><a href="#" class="btn green lighten-2 downloadConfig"><i class="material-icons" style="color: #000 !important;">archive</i></a></td>
                    </tr>`
                    );
                }
                ...
                $('.downloadConfig').click(function (e) { 
                    ...
                    zip.file('pre.bat', initPre + paramEquipment + finalPre + paramClient + task + paramTask);
                    zip.file('post.bat', initPost + paramEquipment + finalPost + paramClient + task + paramTask);
    ...
    // 
    // AL HACER CLICK EN BOTON lastRegister EJECUTA LA FUNCION Y SE EJECUTA AUTOMATICAMENTE CADA 1 MINUTO
    ...
    $('.dashboard').click(function (e) { 
        e.preventDefault();
        $('.dashboard-panel').css({'background-color': '#f2f2f2', 'width': widthScreen, 'height': heightScreen, 'overflow': 'hidden'});
        $.ajax({
            type: "GET",
            ...
            success: function (response) {
                ...
                console.log(row);
                    ...
                    for (let i=0; i < row.length; i++){
                    html.push(
                        `<div>${row[i].businessname} | ${row[i].tasks_count} </div>`
                    );
                }
                $('.rejilla').html(html.join(''));
            }
        });
        $('.rejilla').css({'display': 'grid', 'grid-template-columns': '1fr 1fr 1fr 1fr'});
    });
    // 
    // BUSCADORES
